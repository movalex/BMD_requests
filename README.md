...
28. Bring back the possibility to switch in the lower part between different aspect like nodes, spline, key frames. The current solution when you when you have half of the space for each function its not ideal, there is not enough space to work. So to go from the Nodes to Spline and back we have to do 4 clicks to instead of two. Adding shortcut would also useful. 

29. The scaling on 4K monitors is problematic in Fusion and also in Resolve, there is only two option 100% and 200%. On 4K laptop screen the scaling looks fine but on 32 inch 4K monitor the 100% is to small to see and 200% is to big taking avay lot of space with no use that could be used to view the image or navigate the node graph. 
